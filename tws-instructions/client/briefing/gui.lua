...
gui.alphaStep = 10
gui.borderSize = 1
gui.font = dxCreateFont("stuff/OpenSans-Regular.ttf", 12)
...
arrow.texture = dxCreateTexture("stuff/arrow.png", "argb", true, "clamp")
...
addEvent("initInstructions", true)
addEvent("deinitInstructions", true)
...
function init()
	localPlayer:setData("instructions.initialized", true)
...
addEventHandler("initInstructions", resourceRoot, init)

function deinit()
	localPlayer:setData("instructions.initialized", false)

	isInitialized = false

	removeEventHandler("onClientRender", root, clientRender)
end
addEventHandler("deinitInstructions", resourceRoot, deinit)
