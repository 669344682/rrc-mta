...
function replaceTexture(name, path)
	local texture = dxCreateTexture(path)
	local shader = dxCreateShader("shaders/texreplace.fx")
...
function toggleCustomTiles ( )
	...
	if enabled then
		...
		replaceTexture("radar_waypoint", "images/icons/waypoint.png")
		--replaceTexture("radar_centre", "images/icons/arrow.png")
...
function loadTile ( name )
	...
	-- Fetch the filename
	local file = "images/map/"..name .. ".png"--string.format ( "images/map/sattelite_%d_%d.jpeg", row, col )
